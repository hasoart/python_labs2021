...
class Gun:
    ...
    def move(self, direction):
        ...
        arguments:
        direction(str) - направление движения. Передается ГЕЙмерским методом(wasd)
        """
...
class TargetHorizontal(Target):
    ...
    def move(self, dt=1):
        ...
        if not -self.r < self.x < WIDTH + self.r:
            self.x = (1 - self.r) if (self.x >= WIDTH + self.r) else (WIDTH + self.r - 1)
...
class TargetVertical(Target):
    ...
    def move(self, dt=1):
        ...
        if not -self.r < self.y < HEIGHT + self.r:
            self.y = (1 - self.r) if (self.y >= HEIGHT + self.r) else (HEIGHT + self.r - 1)
...
while True:
    ...
    for i in range(leaderboard_length):
        ...
        bg_color = WHITE if i % 2 else LIGHT_GREY
        row.fill(bg_color)
        name_surface = small_font.render(f'{name}', True, BLACK)
        score_surface = small_font.render(f'{score}', True, BLACK)
        row.blit(name_surface, (3, 3))
        row.blit(score_surface, (203, 3))

        pygame.draw.rect(row, BLACK, (0, 0, 300, 30), width=3)
    ...
    while not retry:
        ...
        pygame.draw.rect(screen, GREEN, (600, 400, 200, 200))
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                exit_program()
